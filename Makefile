...
run: ## runs the api locally via **docker**
	docker compose -f docker-compose.yml up --build run-model-result-checker

.PHONY: create-migration
create-migration: ## create migration with <migration_name> via **docker**
	docker compose -f docker-compose.yml run --rm --no-deps model-result-checker poetry run flask db migrate -m "$(MIGRATION_NAME)"